...
**This textbook is a Level 2.99 Intelligent Textbook.** It includes interactive MicroSims, self-check quizzes, a searchable glossary, FAQ, annotated references, and a learning graph — but it purposly
excludes or adaptive learning paths that are customized to a students learning history because
this version does not store student-specific data.

### What Makes This Textbook Different

- **Interactive MicroSims** let students manipulate economic models directly in their browser — no software installation required
- **Critical thinking emphasis** — every chapter helps students detect misinformation about economics in social media and journalism
- **"Economic superpower" framing** — a positive, empowering tone that makes economics feel relevant to students' daily lives
- **Learning graph** — a visual map showing how all 206 concepts connect and build on each other
- **Ferris the Fox** — a friendly mascot character (called a "pedagogical agent") who guides students through each chapter with tips, encouragement, and key insights
...
The textbook contains **14 chapters** organized in a deliberate sequence. Each chapter builds on concepts from previous chapters, so students should work through them in order. The chapters progress from foundations to microeconomics to macroeconomics to applied topics:
...
3. **Concepts covered** — A numbered list of the specific economics concepts addressed in the chapter, drawn from the learning graph.
4. **Prerequisites** — Links to prior chapters that should be completed first.
5. **Welcome from Ferris** — A mascot admonition that introduces the chapter topic in Ferris the Fox's friendly voice.
6. **Main content** — The core instructional material, written at a Grade 11 reading level. Includes tables, real-world examples, and embedded MicroSims.
7. **Mascot admonitions** — Throughout the chapter, Ferris appears 5–6 times to highlight key insights (thinking), offer practical tips (tip), provide encouragement on harder concepts (encourage), and warn about common mistakes (warning).
8. **Key takeaways** — A numbered summary of the most important concepts, preceded by a celebration from Ferris.
9. **Critical thinking challenge** — Expandable sections that train students to spot economic misinformation.
10. **Practice questions** — Open-ended questions for discussion or written responses.

### Suggested Classroom Use

- **Before class**: Assign the chapter as reading homework. The MicroSims and Ferris admonitions keep students engaged during independent reading.
...
- **Pacing**: Each chapter is designed for approximately 2–3 class periods (90–135 minutes of instruction). Some chapters with more MicroSims (like Chapter 4: Market Equilibrium) may take longer.
...
Each MicroSim lets students manipulate one or more variables (using sliders, buttons, or drag-and-drop) and immediately see how the economic model responds. This "learn by doing" approach helps students build intuition for abstract concepts.
...
The textbook includes over 70 MicroSims built with different visualization technologies:

| Technology | What It's Good For | Example MicroSims |
|-----------|-------------------|-------------------|
| **p5.js** | Interactive animations with sliders and buttons | Supply and Demand, PPF Explorer, Budget Constraint |
| **Chart.js** | Bar charts, line charts, pie charts | GDP Components, Inflation Rate, Stock Market Returns |
| **Plotly** | Advanced interactive charts with hover details | Yield Curve, Trade Balance |
| **vis-network** | Network diagrams showing connections | Learning Graph Viewer, Related Goods Network |
...
!!! mascot-tip "Ferris's Tip: Embed MicroSims Anywhere!"
    ...
    Replace `supply-and-demand` with the name of any MicroSim from
...
The **glossary** is an alphabetical list of all key economics terms used in the textbook, each with a precise, concise definition. It serves as a quick-reference dictionary for students encountering unfamiliar vocabulary.
...
- **Glossary quizzes** — Use glossary terms for quick formative assessments (flash cards, Kahoot-style games, etc.).

## Using the FAQ

### What is the FAQ?

The **FAQ** (Frequently Asked Questions) is a curated list of common questions students ask about economics, organized by topic. Each question includes a clear, concise answer written at the same reading level as the chapters.

### How the FAQ is Organized

The FAQ covers questions across all 14 chapters, from foundational questions like "What is the difference between microeconomics and macroeconomics?" to applied questions like "How does compound interest work?" Questions are grouped by topic area to make browsing easy.
...
1. Go to the textbook's GitHub repository: [github.com/dmccreary/economics-course](https://github.com/dmccreary/economics-course)
2. Click the **"Issues"** tab at the top of the page
3. Click the green **"New issue"** button
4. Give your issue a clear title (e.g., "Broken link in Chapter 5 references" or "Suggestion: Add MicroSim for tax incidence")
...
1. Go to [github.com/dmccreary/economics-course](https://github.com/dmccreary/economics-course)
...
```yaml
site_name: "Your Custom Economics Textbook"
...
theme:
  palette:
    - scheme: default
      ...
      accent: yellow    # Change the accent color
```
...
4. Open your browser to `http://127.0.0.1:8000/economics-course/` to see your customized version
...
This command builds the website and publishes it to `https://YOUR-USERNAME.github.io/economics-course/`. The process takes about 1–2 minutes.
...
**xAPI** (Experience API, also called "Tin Can API") is an advanced standard for tracking detailed learning activities — not just page views, but specific interactions like "student moved the supply curve slider to position X" or "student answered quiz question 3 correctly."
...
For example, the concept "Price Elasticity of Demand" depends on understanding "Demand Curve," which depends on "Law of Demand," which depends on "Demand." The learning graph makes these dependency chains visible.
...
## Ferris the Fox: Your Pedagogical Agent
...
Ferris the Fox appears as colored callout boxes (called **admonitions**) throughout each chapter. There are seven types:
...
Ferris appears no more than 5–6 times per chapter to avoid overuse. Mascot admonitions are never placed back-to-back.

### Tips for Teachers

- **Read Ferris's tips aloud** — They're written in a conversational tone that works well when spoken
- **Use as discussion prompts** — Ferris's "thinking" admonitions highlight the most important insights in each chapter
- **Encourage struggling students** — Point students to Ferris's "encourage" admonitions when they're frustrated with a concept
